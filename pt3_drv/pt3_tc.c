...
tc_read(PT3_TC *tc, PT3_BUS *bus, __u8 addr, __u8 *data, __u32 size)
{
	...
	__u8 buf[size];
	__u32 i, rindex;
	PT3_BUS *p = bus ? bus : create_pt3_bus();
...
{
	STATUS status = write_pskmsrst(tc, bus);
	__u8 buf = 0x10;

	if (status)
		return status;
	return pt3_tc_write(tc, bus, 0x1e, &buf, 1);
}
...
pt3_tc_set_powers(PT3_TC *tc, PT3_BUS *bus, int tuner, int amp)
{
	...
	return pt3_tc_write(tc, bus, 0x1e, &data, 1);
}
...
pt3_tc_set_agc_s(PT3_TC *tc, PT3_BUS *bus, PT3_TC_AGC agc)
{
	...
	if ((status = pt3_tc_write(tc, bus, 0x0a, &data, 1)))
		return status;
	
	data = agc_data_s[pt3_tc_index(tc)];
	data |= (agc == PT3_TC_AGC_AUTO) ? 0x01 : 0x00;
	if ((status = pt3_tc_write(tc, bus, 0x10, &data, 1)))
		return status;

	data = (agc == PT3_TC_AGC_AUTO) ? 0x40 : 0x00;
	if ((status = pt3_tc_write(tc, bus, 0x11, &data, 1)))
		return status;
	return write_pskmsrst(tc, bus);
}
...
{
	STATUS status;
	__u8 data = (agc == PT3_TC_AGC_AUTO) ? 64 : 0;

	if ((status = pt3_tc_write(tc, bus, 0x25, &data, 1)))
		return status;

	data = 0x4c;
	data |= (agc == PT3_TC_AGC_AUTO) ? 0x00 : 0x01;
	if ((status = pt3_tc_write(tc, bus, 0x23, &data, 1)))
		return status;
	return write_imsrst(tc, bus);
}
...
{
	__u8 buf = sleep ? 1 : 0;
	return pt3_tc_write(tc, bus, 0x17, &buf, 1);
}
...
{
	__u8 data;
	STATUS status = tc_read(tc, bus, 0x80, &data, 1);
	if (!status) {
...
pt3_tc_read_tmcc_s(PT3_TC *tc, PT3_BUS *bus, TMCC_S *tmcc)
{
	...
	if ((status = tc_read(tc, bus, 0xc3, data, 1)))
		return status;
	if (BIT_SHIFT_MASK(data[0], 4, 1))
		return STATUS_GENERAL_ERROR;
	
	if ((status = tc_read(tc, bus, 0xce, data, 2)))
		return status;
	if (byte2(data) == 0)
		return STATUS_GENERAL_ERROR;
	...
	if ((status = tc_read(tc, bus, 0xc5, data, SIZE)))
		return status;
...
pt3_tc_read_tmcc_t(PT3_TC *tc, PT3_BUS *bus, TMCC_T *tmcc)
{
	...
	if ((status = tc_read(tc, bus, 0xb2+0, &data[0], 4)))
		return status;
	if ((status = tc_read(tc, bus, 0xb2+4, &data[4], 4)))
		return status;
...
pt3_tc_write_id_s(PT3_TC *tc, PT3_BUS *bus, __u16 id)
{
	...
	return pt3_tc_write(tc, bus, 0x8f, data, sizeof(data));
}
...
{
	__u8 data[2];
	STATUS status = tc_read(tc, bus, 0xe6, data, sizeof(data));
	if (!status)
		*id = byte2(data);
	return status;
}
...
pt3_tc_write_slptim(PT3_TC *tc, PT3_BUS *bus, int sleep)
{
	...
	return pt3_tc_write(tc, bus, 0x03, &data, 1);
}
...
{
	__u8 data;
	STATUS status = tc_read(tc, bus, 0xba, &data, 1);
	if (!status)
		*agc = data & 0x7f;
	return status;
}
...
{
	return tc_read(tc, bus, 0x82, ifagc_dt, 1);
}
...
{
	__u8 data[2];
	STATUS status = tc_read(tc, bus, 0xbc, data, sizeof(data));
	if (!status)
		*cn = byte2(data);
	return status;
}
...
{
	__u8 data[3];
	STATUS status = tc_read(tc, bus, 0x8b, data, sizeof(data));
	if (!status)
		*cn = byte3(data);
	return status;
}
...
{
	PT3_TC *tc = pt3_vzalloc(sizeof(PT3_TC));
	if (!tc)
		return NULL;
...
{
	__u32 diff = (et->tv_sec - st->tv_sec) * 1000000 + (et->tv_usec - st->tv_usec);
#if 0
