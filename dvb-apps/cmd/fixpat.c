/*
 * reads from stdin, finds PAT, rewrites it to contain just one program,
 *  and outputs each packet to stdout.
 * example: ./fixpat 12345  < in-file > out-file
 */
#include <stdio.h>
#include <stdint.h>
#include <stdlib.h>
...
//	unsigned short pid;
	...
	for (q = p + 8; q < end; q += 4)
		if ((q[0] << 8 | q[1]) == sid)
			break;
	if (q >= end) {
		fprintf(stderr, "PAT does not contain sid:0x%04lx.\n", sid);
		return;
	}
...
main(int argc, char **argv)
{
	...
	if (argc != 2 ||
		...
		fprintf(stderr, "Usage: %s \"ServiceID\" "
			"< \"IN-FILE\" > \"OUT-FILE\"\n", argv[0]);
		return 1;
	}
	...
	for(;;) {
		...
		r = fread(buf + 1, sizeof(buf) - 1, 1, stdin);
		if (r < 1)
			 return 0;
		...
		r = fwrite(buf, sizeof(buf), 1, stdout);
		if (r != 1) {
			perror("Failed to output to stdout");
			return 1;
		}
	}
	return 0;
