...
int tda2014x_r(struct dvb_frontend *fe, u8 slvadr)
{
	...
	u8	buf[]	= {0xFE, 0xA8, slvadr},
		...
		ret	= 0;
	struct i2c_msg msg[] = {
		{.addr = fe->id << 8,		.flags = 0,		.buf = buf,	.len = 3,},
		{.addr = (0x80 | fe->id) << 8,	.flags = 0,		.buf = rcmd,	.len = 2,},
		{.addr = fe->id << 8,		.flags = I2C_M_RD,	.buf = &ret,	.len = 1,},
	};
...
bool tda2014x_w16(struct dvb_frontend *fe, u16 slvadr, u8 start_bit, u8 nbits, u8 nbytes, bool rmw, u8 access, u16 wdat)
{
	...
	for (i = 0, nbytes = !nbytes ? 1 : nbytes > 2 ? 2 : nbytes; access & 2 && nbytes; i++, nbytes--) {
		...
		struct i2c_msg msg[] = {
			{.addr = fe->id << 8,	.flags = 0,	.buf = buf,	.len = 4,},
		};
...
static struct dvb_tuner_ops tda2014x_ops = {
	.info = {
		.frequency_min	= 1,		/* freq under 1024 kHz is handled as ch */
		.frequency_max	= 3000000,	/* kHz */
...
bool nm131_w(struct dvb_frontend *fe, u16 slvadr, u32 val, u32 sz)
{
	...
	struct i2c_msg msg[] = {
		{.addr = fe->id << 8,	.flags = 0,	.buf = buf,	.len = sz + 4,},
	};
...
bool nm131_r(struct dvb_frontend *fe, u16 slvadr, u8 *dat, u32 sz)
{
	...
	struct i2c_msg msg[] = {
		{.addr = (0x80 | fe->id) << 8,	.flags = 0,		.buf = rcmd,	.len = 2,},
		{.addr = fe->id << 8,		.flags = I2C_M_RD,	.buf = buf,	.len = sz,},
	};
...
bool tc90522_i2c_r_1(struct dvb_frontend *fe, u8 slvadr, u8 *rdat)
{
	...
	struct i2c_msg msg[] = {
		{.addr = (0x80 | fe->id) << 8,	.flags = 0,		.buf = &slvadr,	.len = 1,},
		{.addr = fe->id << 8,		.flags = I2C_M_RD,	.buf = rdat,	.len = 1,},
	};
...
bool tc90522_i2c_w_tuner(struct dvb_frontend *fe, u8 slvadr, u8 dat)
{
	...
	u8 buf[] = {slvadr, dat};
	struct i2c_msg msg[] = {
		{.addr = fe->id << 8,	.flags = 0,	.buf = buf,	.len = 2,},
	};
...
u32 tc90522_get_cn(struct dvb_frontend *fe)
{
	...
	u8	cn[3]	= {0};
	struct i2c_msg msg[] = {
		{.addr = ((0x80 | fe->id) << 8) + 0xBC,	.flags = I2C_M_RD,	.buf = cn,	.len = 2,},
		{.addr = ((0x80 | fe->id) << 8) + 0x8B,	.flags = I2C_M_RD,	.buf = cn,	.len = 3,},
	};
...
bool tc90522_gpio_w_2(struct pxq3pe_card *card, u8 Data, u8 mask)
{
	u8	val;
	struct i2c_msg msg[] = {
		{.addr = (PXQ3PE_I2C_ADR_GPIO << 8) + 0xB,	.flags = I2C_M_RD,	.buf = &val,	.len = 1,},
		{.addr = (PXQ3PE_I2C_ADR_GPIO << 8) + 0xB,	.flags = 0,		.buf = &val,	.len = 1,},
	};
...
bool pxq3pe_tune(struct pxq3pe_adap *p, int fno, int slot)
{
	...
	if (fe->dtv_property_cache.delivery_system == SYS_ISDBS) {
		u32 kHz = fno < 12 ? 11727480 + fno * 38360 : 12291000 + (fno - 12) * 40000;

		fe->dtv_property_cache.frequency = kHz - 10678000;
		tc90522_i2c_w_tuner(fe, 0xA, 0)		&&
...
dev_dbg(&p->card->pdev->dev, "%s TSID %04X", __func__, tsid[i]);
		...
		if (tsid[slot]) {
			tc90522_i2c_w_tuner(fe, 0x8F, tsid[slot] >> 8) &&
			tc90522_i2c_w_tuner(fe, 0x90, tsid[slot] & 0xFF);
		}
	} else {
		u32 MHz = fno > 112 ? 557 : 93 + 6 * fno + (fno < 12 ? 0 : fno < 17 ? 2 : fno < 63 ? 0 : 2);

		fe->dtv_property_cache.frequency = MHz * 1000000 + 142857;
...
void pxq3pe_remove(struct pci_dev *pdev)
{
	...
	if (card->dma.dat) {
		if (!(ops = pdev->dev.archdata.dma_ops))
			ops = dma_ops;
		if (ops->free)
		ops->free(&pdev->dev, card->dma.sz, card->dma.dat, card->dma.adr, NULL);
	}
...
static int pxq3pe_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id)
{
	...
	if (dev->dma_mask && *dev->dma_mask && ops->alloc) {
		...
		if (!dev->coherent_dma_mask ||
			(gfp = 0x21 - (u32)(dev->coherent_dma_mask >= 0x1000000), dev->coherent_dma_mask <= 0xFFFFFFFF))
			if (!(gfp & 1))
				gfp |= 4;
