/*
 * reads from stdin, makes jumped PTS consecutive.(restamp) &&
 *  fix the 0x offset PCR bug of gstreamer 0.10.3x
 * example: ./fixpcr < in-file > out-file
 */
#include <stdio.h>
#include <inttypes.h>
#include <string.h>
...
get_pcr(unsigned char *p)
{
	...
	base = ((uint64_t)(p[0] - 6) << 25) | ((uint64_t)p[1] << 17) |
