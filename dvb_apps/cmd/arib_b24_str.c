...
void g_string_append_c (gstr *euc_str, uint8_t c)
{
  if (euc_str->used < euc_str->len)
    euc_str->buf[euc_str->used++] = c;
}

void g_string_append (gstr *euc_str, const char *txt)
{
...
void append_arib_char (gstr *euc_str, const struct code_state *state,
...
void aribstr_to_utf8 (char *source, size_t len, char *dest, size_t buf_len)
{
  int i, idx = 0;
  uint8_t prev = '\0';
  gstr euc_str;
